...
export async function getWorkdays(filters: WorkdayFilters = {}): Promise<Workday[]> {
  const { branch_id, courier_id, date, date_from, date_to, status, page, page_size } = filters
  ...
  // 2. Carga en lote de movimientos de caja solo para las jornadas de la página actual
